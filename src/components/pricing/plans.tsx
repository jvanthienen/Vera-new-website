...
export function Plans() {
  ...
  const handlePlanChange = (priceId: string) => {
    posthog.capture("pricing_plan_selected", { price_id: priceId });
    setSelectedPriceId(priceId);
  };

  return (
    <div className="relative z-10 mx-auto flex w-full max-w-md flex-col items-center">
      <h1 className="mt-8 mb-4 font-serif text-3xl font-bold md:text-center md:text-4xl">
      ...
      <PlanSelect value={selectedPriceId} onChange={handlePlanChange} />
      <Button
        className="mb-8 w-full"
        size="lg"
        asChild
        onClick={() => {
          posthog.capture("checkout_button_clicked", { price_id: selectedPriceId });
        }}
      >
